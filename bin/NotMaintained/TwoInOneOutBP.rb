...
class Experiment
  ...
  def createSimplest2GaussianClustersForTrainAndTest(numberOfExamples)
    ...
    classIndexAndClassRNG = [(classIndex = 0), (classRNG = negativeNormalCluster), (mean = -1.0)]
    arrayOfClassIndexAndClassRNGs << classIndexAndClassRNG
    classIndexAndClassRNG = [(classIndex = 1), (classRNG = positiveNormalCluster), (mean = 1.0)]
    arrayOfClassIndexAndClassRNGs << classIndexAndClassRNG
    ...
    arrayOfClassIndexAndClassRNGs.each do |classIndexAndClassRNG|
      ...
      numberOfExamplesInEachClass.times do |classExNumb|
        ...
        x0 = randomNumberGenerator.rng
        x1 = 0.1 * (x0 - classIndexAndClassRNG[2])
        aPoint = [x0, x1]
...
experiment = Experiment.new("NonM1std2in1out 2-Class classifier, 2 gaussian clusters (1.0 std) with NonMonotonic IO function for simple 2 in 1 out network", baseRandomNumberSeed)
